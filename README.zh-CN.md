...
首次运行时，会自动在 `~/.websearch-via-deepseek/settings.json` 创建用户配置文件。
...
或编辑 `~/.websearch-via-deepseek/settings.json`，填入 `apiKey`。
...
3. **用户配置文件** — `~/.websearch-via-deepseek/settings.json`
4. **默认值** — 内置于服务端
